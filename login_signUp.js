...
let read = f.readFileSync('userData.txt', 'utf-8')
if (read.includes(email)) {
    console.log('you hava already signUP. please login .');

} else {
    let password = input.question('Apna Password dale : ')
    let a = f.appendFileSync('userData.txt', `${email},${password},${name}\n`)
...
do{
    ...
    if (read.includes(email)) {
        ...
        console.log(`you already have an account with this ${email} id\n try again`);
        continue
...
//***************************************** login/signup with proper email and password (text file)?
